...
const CreateShort = ({ onSave, onFinish, setStep }) => {
  const [question, setQuestion] = useState("");

  const handleSaveOnly = () => {
    if (!question.trim()) return alert("Question cannot be empty");
    onSave({ question, type: "SHORT", marks: 5, options: [], correctAnswer: "" });
    setQuestion("");
    alert("Short Question Saved!");
  };
  ...
  return (
    <div style={styles.container}>
      <h2>SECTION : SHORT ANSWER</h2>
      <div style={styles.card}>
        <label style={{ fontWeight: "bold" }}>Question :</label>
        <input style={styles.input} value={question} onChange={(e) => setQuestion(e.target.value)} placeholder="Enter Short Answer Question" />
        <div style={styles.buttonGroup}>
          <button style={{ ...styles.btn, backgroundColor: "#4A5568", color: "white" }} onClick={handleSaveOnly}>Save & Add Another</button>
          <button style={{ ...styles.btn, backgroundColor: "#E53E3E", color: "white" }} onClick={() => setStep("LONG")}>Move to Long Answer</button>
